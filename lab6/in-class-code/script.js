...
function gotData(incomingData){
  ...
  function visualizeCurrentData(){
    ...
    enteringGroups.append("text")
        ...
        .attr("fill", "white")
    ;
    enteringGroups.attr("transform", getEnteringPosition).transition().duration(200).attr("transform", getGroupPosition);

    // UPDATING GROUPS
    datagroups.transition().delay(200).duration(200).attr("transform", getGroupPosition);

    // EXITING GROUPS
    let exitingGroups = datagroups.exit();
    exitingGroups.transition().delay(400).duration(200).attr("transform", getExitingPosition).remove();
